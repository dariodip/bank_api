defmodule BankAPI.Accounts.Commands.TransferBetweenAccounts do
  ...
  def valid?(command) do
    ...
    with %Account{} <- account_exists?(cmd.destination_account),
         true <- account_open?(cmd.destination_account) do
      Skooma.valid?(cmd, schema())
    else
      nil -> {:error, ["Destination account does not exist"]}
      false -> {:error, ["Destination account closed"]}
      reply -> reply
    end
  end
